...
The spec root directory name is configurable via the `--spec-root` flag (default: `spec`). All path resolution — scenario discovery, AC resolution, sub-flow includes — uses this configured root.
...
| Concern | Gherkin/Cucumber | YAML test configs | Rehearse markdown |
|---|---|---|---|
