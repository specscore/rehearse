...
Rehearse's scenario format avoids all three traps: human-readable markdown that renders anywhere, bash blocks that execute directly (no glue code), and native AC composition that references verification logic instead of reimplementing it.
...
Use `manual` for demo scenarios, long-running stress tests, or interactive verification that should not execute in CI or default `rehearse run` sweeps. Scenarios tagged `manual` still appear in `rehearse list` output — they are discoverable but opt-in for execution.
