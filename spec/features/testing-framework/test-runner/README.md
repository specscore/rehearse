...
The package is self-contained with no framework-specific dependencies. It depends only on the Go standard library and a markdown parser. This means it can be embedded in other projects or tested in isolation.
...
The runner reads the `--spec-root` flag (default: `spec`). All AC references resolve relative to this root — `cli/project/remove/*` becomes `{spec_root}/features/cli/project/remove/_acs/`. Configuration is read once at initialization and threaded through the AC resolver.
...
Scenarios tagged `manual` are excluded from directory-scanned runs by default. This prevents demo scenarios, stress tests, and interactive verification from running in CI or routine `rehearse run` sweeps.

Manual scenarios run when:
- The scenario file is **passed directly by path** (e.g., `rehearse run path/to/demo.md`)
